## random forest model (samo soncen elektrarne)
	...
	if(substr(kraj, 1, 5) == 'vreme'){
		...
		for(elekt in dir()){
			...
			gsub(".*T]|_.*", "", elekt) == 'Sonce'){
				...
				A <- na.omit(A)
				## ucenje modela
				model <- randomForest(A[,1:(ncol(A)-1)], A[, ncol(A)], ntree = stDreves) ## glej clanek
				## validacija modela
				...
				setwd(Okolje1)
				## ime modela
				imeModel <- paste0(substr(elekt, 1, 8), '_model_', stDreves,'_RF_cist.rds')
				saveRDS(model, imeModel)
				## shranjevanje validacije
				setwd(Okolje2)
				## ime validacije
				imeValid <- paste0(substr(elekt, 1, 8), '_validacija_', stDreves,'_RF_cist.rds')
