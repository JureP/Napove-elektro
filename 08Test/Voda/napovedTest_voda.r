## napovedi modelov na testni mnozici (VODA)
library(xts)
...
## okolje kjer so feature matrike testni dela mnozice
OkoljeFM <- 'C:/Users/Podlogar/Documents/Projekt Elektro/00_Podatki/featureMatrix_voda/Test'
## okolje kamor naj se shranijo mape (pozameznih modlev) z napovedmi
OkoljeSaveD <- 'C:/Users/Podlogar/Documents/Projekt Elektro/08_Test(voda)/NapovedTest'
...
for(elekt in dir()){
	...
	if (gsub(".*T]|_.*", "", elekt) == 'Voda'){
		...
		for(mdl in dir()){
			...
			setwd(OkoljeSave)
			imeSave <- paste0(substr(elekt,1,8),'_', mdl, '_napovedTest.rds')
